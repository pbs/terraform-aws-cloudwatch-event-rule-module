...
```hcl
module "rule" {
    source = "github.com/pbs/terraform-cloudwatch-event-rule-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/cloudwatch-event-rule'
gh -R 'pbs/terraform-cloudwatch-event-rule-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/cloudwatch-event-rule'
rm -f release.tar.gz
```
...
```bash
git remote add -f cloudwatch-event-rule git@github.com:pbs/terraform-cloudwatch-event-rule-module.git
git subtree add --prefix terraform/modules/cloudwatch-event-rule cloudwatch-event-rule main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/cloudwatch-event-rule cloudwatch-event-rule $REF --squash
```
